...
export async function autoSetupDatabase(pool) {
  if (!pool) {
    console.log('⏭️ Auto-setup skipped: No database pool available');
    return false;
  }
  ...
  try {
    ...
    if (existingOwner.rows.length === 0) {
      ...
      const result = await pool.query(
        ...
        [DEFAULT_OWNER.username, emailHash, passwordHash, DEFAULT_OWNER.username, DEFAULT_OWNER.role]
      );

      const userId = result.rows[0].id;

      await pool.query(
        'INSERT INTO profiles (user_id, bio, avatar_url, theme) VALUES ($1, $2, $3, $4) ON CONFLICT (user_id) DO NOTHING',
      ...
      if (updateResult.rows.length > 0) {
        const currentHash = updateResult.rows[0].password_hash;
        const isCorrect = await bcrypt.compare(DEFAULT_OWNER.password, currentHash);
        
        if (!isCorrect) {
          console.log('🔄 Updating owner account password...');
          const newPasswordHash = await bcrypt.hash(DEFAULT_OWNER.password, 12);
          await pool.query(
            'UPDATE users SET password_hash = $1 WHERE username = $2',
            [newPasswordHash, DEFAULT_OWNER.username]
          );
          console.log('✅ Owner account password updated');
        } else {
          console.log('✅ Owner account already exists with correct password');
        }
      }
    }
