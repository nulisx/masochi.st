...
// Only run seeding if explicitly enabled. This avoids failing builds
// when the database is not reachable during deploys. Set RUN_SEED=true
// in your environment to enable seeding.
...
async function seedOwnerAccount() {
  ...
  try {
    ...
    // Do not fail the build/deploy; just log and continue.
  } finally {
    if (connection) {
